...
import { eat } from "@store/slices/historySlice"
...
function Product({ name, b, zh, u, k }: propsProd) {
  ...
  function mat(p: number, s: string, k?: number) {
    return k && ei === "Шт"
      ? Math.round((p / 100) * Number(s) * k)
      : Math.round((p / 100) * Number(s))
  }
  ...
  return (
    <div className={`${cls.main}`}>
      <div>
        {name}
        <br />К{Math.ceil(b * 4 + zh * 9 + u * 4)} Б{b} Ж{zh} У{u}
      </div>
